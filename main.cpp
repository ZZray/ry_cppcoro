...
RyCoroTask<int> longRunningTask()
{
	auto formatTime = [](std::chrono::system_clock::time_point tp) {
		...
		std::tm localtime = *std::localtime(&time); // 使用 std::localtime 并复制 tm 结构体
		std::stringstream ss;
		ss << std::put_time(&localtime, "%Y-%m-%d %H:%M:%S");
		return ss.str();
		};
	using namespace std::literals;
	std::cout << "开始长时间运行的任务 " << formatTime(std::chrono::system_clock::now()) << '\n';
	co_await 5s;
	std::cout << "长时间运行的任务完成" << formatTime(std::chrono::system_clock::now()) << '\n';
	co_return 100;
}
